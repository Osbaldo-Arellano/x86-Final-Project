...
;              to get user input and display numbers in the form of a BYTE string array.
;              After converting numbers to their ASCII representation,
;               the number array, total sum, and truncated average is displayed.
;
;             NOTE: Some of this functionality is not implemented. 
...
; Name: mGetString
;
; Gets input from the user and stores it into input byte array
;
; Preconditions: none
;
; Receives: mssg   = pointer to some prompt message
;           input  = pointer to array that will hold user input
;           buffer = max byte array length
;           length = Will hold length of user input 
;
; Returns: arrayAddr = generated byte array address
;          length = length of generatyed byte array
; ---------------------------------------------------------------------------------
...
; Name: mDisplayString
;
; Displays a byte array (string) 
;
; Preconditions: mssg is byte array
;
; Receives: mssg   = pointer to some message
;
; Returns: none
; ---------------------------------------------------------------------------------
...
displayMssg    BYTE   13,10,"Displaying valid numbers: ",13,10,0
...
input          BYTE	  13 DUP(0)
inputLen       DWORD  ?
resultArray    BYTE   1 DUP(?)
testArray      BYTE   13 DUP(?)
...
_done:	
; ----------------------
; I could not get the numbers to display
; using the WriteVal procedure.
; 
; For displaying purposes, I'm using WriteInt to
; display the numbers array.
;
; I dont expect to get credit for this section.
; ----------------------
	push    LENGTHOF validArray
	push    OFFSET displayMssg
	push    OFFSET validArray
	call	displayList
	call	CrLf

; -----------------------
; getSum is not implemented correctly. 
; Currently, it displays the sum of the numbers in reverse order 
; -----------------------
...
;
; Preconditions: Array of type SDWORD. 
;
...
;
; Receives: [ebp + 8] = someArray.
;           [ebp + 12] = someTitle.
;           [ebp + 16] = length of someArray. 
;
; Returns: None
; ---------------------------------------------------------------------------------
displayList PROC
...
; Name: getSum
;
; Sums all the numbers in the validArray (SDWORDS)
; Displays the sum as a string of characters. 
;
; Preconditions: Array of type DWORD
;
; Postconditions: Modifies EDI to point to some array. 
;                 Modifies ESI to point to the 
;                 Modifies ECX as the loop counter
;
; Receives: [ebp + 8]  = Reference to message to display to the user
;           [ebp + 12] = Refrence to SDWORD array 
;           [ebp + 16] = length of SWORD array that hold user input.
;           [ebp + 20] = Reference to the byte array that will hold the 
;                        ASCII representation of some value. 
;
; Returns: None
;
;  *** NOTE: Not implemented correctly. Displays the sum of the numbers in reverse order ****
; ---------------------------------------------
getSum PROC
	push    ebp
	mov     ebp, esp
	mov     edi, [ebp + 20]           ; test arr
	mov     ecx, [ebp + 16]           ; Points to length of array
...
_sumLoop:
	...
	mov     ebx, 10                   ; Divide by 10 until quotient is zero 
	mov     eax, edx
_loop:
	cdq
	idiv    ebx
	...
	add     esi, 4
	pop     eax
	cmp     eax, 0                 ; If quotient is zero, break loop
	je      _done
	jmp     _loop
...
;                 Modifies ESI to point to array 
;                 Modifies EAX to hold current number to be converted to ASCII representation
;                 Modifies EDI to point to array
;                 Modifies EBX to hold divisor 
;
...
; 
; preconditions: Array whose elements are of type SDWORD
;
...
writeval proc
	...
	mov     esi, [ebp + 8]          ; points to buffer string 
	mov     eax, [ebp + 12]         ; points to current number
	mov     edi, [ebp + 16]         ; points to testArray
	mov     eax, [eax]
	mov     ebx, 10                 ; Divide by 10 until quotient is zero 
_loop:
	cdq
